...
export default async function DashboardPage() {
  ...
  const user = await fetchUserFromApi(accessToken);
  ...
  const recentActivity = [
    ...
    },
    {
      id: 4,
      type: "order",
      message: "Order #ORD-7841 shipped",
      time: "2 hours ago",
    },
  ];

  // Render — you can keep your UI components here. Client components like LogoutButton are allowed.
  return (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center space-x-4">
              <Link
                href="/"
                className="text-gray-700 hover:text-purple-600 font-medium text-base"
              >
                Home
              </Link>
              ...
              {/* LogoutButton is likely a client component — server can render it */}
              <LogoutButton />
            </div>
          </div>
        </div>
...
/* --------------------------
   Helper UI components (server)
   They are simple — keep them here or move to separate files.
   If some components need client features (hooks, local state), mark them as "use client" inside those files.
   -------------------------- */
...
function ActivityItem({ type, message, time }) {
  ...
  return (
    <div className="flex items-center space-x-3">
      <div className={`p-2 rounded-lg ${getColor(type)}`}>{getIcon(type)}</div>
